...
" Configuration: {{{
let s:configuration = {}
let s:configuration.palette = get(g:, 'skybox_palette', 'soft')
let s:configuration.transparent_background = get(g:, 'skybox_transparent_background', 0)
let s:configuration.disable_italic_comment = get(g:, 'skybox_disable_italic_comment', 0)
let s:configuration.enable_italic = get(g:, 'skybox_enable_italic', 0)
let s:configuration.cursor = get(g:, 'skybox_cursor', 'auto')
let s:configuration.current_word = get(g:, 'skybox_current_word', get(g:, 'skybox_transparent_background', 0) == 0 ? 'grey background' : 'bold')
" }}}
" Palette: {{{
let s:palette = {
      ...
      \ 'yellow':     ['#eef07f',   '214',  'Yellow'],
      \ 'green':      ['#7ff088',   '108',  'Green'],
